...
enum Passes {
  ...
  /// the URL of the file.
  static let readFile =
    BlockPass<URL, (String, URL)>(name: "Read File") { url, ctx in
      do {
  ...
  static let lex =
    BlockPass<(String, URL), [TokenSyntax]>(name: "Lex") { file, _ in
  ...
  static let shine =
    BlockPass<[TokenSyntax], [TokenSyntax]>(name: "Shine") { tokens, _ in
      layout(tokens)
    }
  ...
  static let parse =
    BlockPass<[TokenSyntax], ModuleDeclSyntax>(name: "Parse") { tokens, ctx in
  ...
  static let scopeCheck =
    DiagnosticGatePass(
      BlockPass<ModuleDeclSyntax, DeclaredModule>(name: "Scope Check") { mod, ctx in
  ...
  static let typeCheck =
    DiagnosticGatePass(
      BlockPass<DeclaredModule, Module>(name: "Type Check") { module, _ in
